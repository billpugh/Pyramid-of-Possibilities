...
public class Animation {
    ...
    enum TweakKind {
        SIGNED, UNSIGNED, CYCLIC, CYCLIC_UNARY;
    ...
    class TweakStatus {
        ...
        private TweakStatus(byte cpm, double cycles, double tweakCount,
                int computedAt) {
            this.tweakValue = cpm;
            this.cycles = cycles;
        ...
        TweakStatus tweakUp() {
            ...
            if (newTweakCount < 0)
                ...
                newTweakCount = 8;
            double newCycleCount = cycles + tweakValue * (at - lastTweakAt)
                    / 120000.0;
            int increment = 1;
            if (program.tweakKind != TweakKind.CYCLIC_UNARY
                    && newTweakCount > 5)
                increment = 4;

            return new TweakStatus(
                    program.tweakKind.add(tweakValue, increment),
                    newCycleCount, newTweakCount, at);
        }
        ...
        TweakStatus tweakDown() {
            ...
            return new TweakStatus(
                    program.tweakKind.add(tweakValue, decrement),
                    newCycleCount, newTweakCount, at);
    ...
    public void write(ByteBuffer buf, boolean effect) {
    ...
    public Animation(AnimationProgram program, byte initialTweakValue,
            int duration, byte[] parameters) {
    ...
    public ByteBuffer getBytes() {
        ...
        if (buf.limit() != byteLength())
            throw new AssertionError(String.format(
                    "limit is %d, bytelength is %d", buf.limit(), byteLength()));

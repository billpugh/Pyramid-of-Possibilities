...
void showLEDs() {
  for(int i = 0; i < LEDs; i++) if (l[i] == 0.0)
    ...
    strip.setPixelColor(i, HSVtoRGB(140*temperature*(1+l[i]), 1.0, l[i]));
  }
  strip.show();
}
...
{  
  ...
  if (millis()  > nextUpdate) {
    nextUpdate += 150-120*temperature;
    updateLEDs();
  }
...
void initMMA8452(byte fsr, byte dataRate)
{
  ...
  writeRegister(0x23, 0x00);  // 2. x thresh at 2g, multiply the value by 0.0625g/LSB to get the threshold
  writeRegister(0x24, 0x00);  // 2. y thresh at 2g, multiply the value by 0.0625g/LSB to get the threshold
  writeRegister(0x25, 0x00);  // 2. z thresh at .5g, multiply the value by 0.0625g/LSB to get the threshold
